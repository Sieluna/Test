...
impl<'res> DescriptorSet<'res> {
    ...
    /// ```
    pub fn bind_sampler(mut self, sampler: &'res super::Sampler) -> Self {
        let bind_id = self.set_layout.len() as u32;

        let bind_type = match sampler.filter_mode {
            super::SamplerFilterMode::Linear => wgpu::SamplerBindingType::Filtering,
            super::SamplerFilterMode::Nearest => wgpu::SamplerBindingType::NonFiltering,
        };
